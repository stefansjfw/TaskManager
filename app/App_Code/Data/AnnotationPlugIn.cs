...
namespace StefanTutorialDemo.Data
{
    public class AnnotationPlugIn : IPlugIn
    {
        ...
        void IPlugIn.ProcessPageRequest(PageRequest request, ViewPage page)
        {
            ...
            else
            {
                ...
                a.CommandArgument = "this.get_view()._displayAnnotations=true;this._focusedFieldName = '_Annotation_NoteNew';this._raiseSelectedDelayed=false;";
    ...
    public class AnnotationBlobHandler : BlobHandlerInfo
    {
            ...
            if (m.Success)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        writer.WriteEndElement();
                        fileName = ((Path.GetFileNameWithoutExtension(fileName) + "_") + Path.GetExtension(file.FileName));
        ...
        public override void LoadFile(Stream stream)
        {
            ...
            fileName = Path.Combine(p, ((Path.GetFileNameWithoutExtension(fileName) + "_") + Path.GetExtension(nav.GetAttribute("fileName", string.Empty))));
