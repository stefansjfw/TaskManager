...
namespace StefanTutorialDemo.Data
{
    ...
    public class FilterValue
    {
        ...
        public FilterValue(string fieldName, RowFilterOperation operation, params object[] value) :
    ...
    public class BusinessRulesBase : ActionHandlerBase, StefanTutorialDemo.Data.IRowHandler, StefanTutorialDemo.Data.IDataFilter, StefanTutorialDemo.Data.IDataFilter2
    {
        ...
        private object _theRESTfulResult;
        ...
        public IEnumerable<object> ResultSetArray
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    if ((table.Columns.Count == 0) && hasRows)
                        ResultSet = null;
                    else
                        ResultSet = table;
                }
        ...
        public object ResultSetObject
        {
            get
            {
                if (_resultSetArray == null)
                    return null;
                var enumerator = _resultSetArray.GetEnumerator();
            ...
            set
            {
                if (value == null)
                    ...
                    ResultSetArray = new object[] {
                            value};
            }
        }

        public object RESTfulResult
        {
            get
            {
                return _theRESTfulResult;
            }
            set
            {
                _theRESTfulResult = value;
                ResultSet = null;
                if (value == null)
                    Result.Values.Clear();
                else
        ...
        public DataTable ResultSet
        {
            ...
            set
            {
                ...
                HttpContext.Current.Items["BusinessRules_ResultSet"] = value;
            }
        }
        ...
        {
            var geoFields = Config.Select("/c:dataController/c:views/c:view[@id='{0}']/c:categories/c:category/c:dataFields/c:dataField[contains(@tag, 'geocode-')]", View);
        ...
        public virtual bool Geocode(string address, out decimal latitude, out decimal longitude)
        {
            ...
            {
                var m = Regex.Match(json, "\"location\"\\s*:\\s*{\\s*\"lat\"\\s*:\\s(?'Latitude'-?\\d+.\\d+),\\s*\"lng\"\\s*:\\s*(?'Longitude'-?\\d+.\\d+)");
                if (m.Success)
                {
        ...
        {
            // send request
            var request = WebRequest.Create(string.Format("https://maps.googleapis.com/maps/api/distancematrix/json?origins={0}&destinations={1}&{2}", HttpUtility.UrlEncode(origin), HttpUtility.UrlEncode(destination), ApplicationServices.MapsApiIdentifier));
            var response = request.GetResponse();
            var json = string.Empty;
            using (var sr = new StreamReader(response.GetResponseStream()))
                json = sr.ReadToEnd();
            if (!string.IsNullOrEmpty(json))
            {
                var m = Regex.Match(json, "\"distance\"\\s*:\\s*{\\s*\"text\"\\s*:\\s*\"[\\w\\d\\s\\.]+\",\\s*\"value\"\\s+:\\s+(?'Distance'\\d+)\\s*}");
                if (m.Success)
        ...
        protected void RegisterAccessControlRule(string fieldName, AccessPermission permission, params object[] values)
        {
            if (!_page.ContainsField(fieldName))
                return;
            var a = CreateDynamicAccessControlAttribute(fieldName);
        ...
        protected int Sql(string text, string connectionStringName, params ParameterValue[] parameters)
        {
            ...
            if (!EnableResultSet && text.Contains("BusinessRules_EnableResultSet"))
                EnableResultSet = true;
            if (EnableResultSet && (ResultSetCacheDuration > 0))
            {
                resultSetCacheVar = (("ResultSet_" + _page.Controller) + ("_" + _page.View));
            ...
            {
                var paramRegex = new Regex(string.Format("({0}(?'FieldName'\\w+?)_(?'ValueType'OldValue|NewValue|Value|Modified|FilterValue\\d?|FilterOperation|Filter_\\w+))|({0}(?'FieldName'\\w+))", Regex.Escape(query.ParameterMarker)), RegexOptions.IgnoreCase);
                ...
                while (m.Success)
                {
                    ...
                    if (!names.Contains(paramName.ToLower()))
                    {
                        names.Add(paramName.ToLower());
                ...
                else
                {
                    if (EnableResultSet)
                    {
                        ...
                        SqlResolveOutputParameters(query, buildingRow, parameters);
                        return 0;
                    }
                    else
                    {
        ...
        protected virtual void SqlResolveOutputParameters(SqlText query, bool buildingRow, ParameterValue[] parameters)
        {
                ...
                else
                {
                    if (propertyName.StartsWith("Profile_"))
                    {
                        if (ApplicationServices.IsSiteContentEnabled)
                        {
                            ...
                            if (profile == null)
                            {
                                profile = SiteContentFile.ReadJson((("sys/users/" + UserName) + ".json"));
        ...
        {
            type = DbType.String;
            var m = Regex.Match(name, "^(.+)_As(AnsiString|Binary|Byte|Boolean|Currency|Date|DateTime|Decimal|Double|Guid|Int16|Int32|Int64|Object|SByte|Single|Time|UInt16|UInt32|UInt64|VarNumeric|AnsiStringFixedLength|StringFixedLength|StringFixedLength|Xml|DateTime2|DateTimeOffset)$", RegexOptions.IgnoreCase);
            if (m.Success)
            {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(s))
                                    {
                                        ...
                                        if (nameWithoutMarker == "Result_ReturnValue")
                                        {
                                            var v = p.Value;
                                            if (DBNull.Value.Equals(v))
                                                v = null;
                                            ...
                                            {
                                                var stringValue = Convert.ToString(v);
                                                long longValue = 0;
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            if (DateTime.TryParse(stringValue, out dateTimeValue))
                                                                v = dateTimeValue.ToString("o");
                                                        }
                                                    }
                                                }
                                                Result.Values.Add(new FieldValue("ReturnValue", v));
                                            }
                                        }
                                    }
                                }
        ...
        {
            return ApplicationServices.Create().IsSystemController(controller);
        }
    }
