...
namespace StefanTutorialDemo.Data
{
    ...
    public partial class DataControllerBase : IDataController, IAutoCompleteManager, IDataEngine, IBusinessObject
    {
        ...
        {
            return StringToValue(((string)(null)), s);
        }
        ...
        public static object StringToValue(DataField field, string s)
        {
            ...
            return StringToValue(fieldType, s);
        }

        public static object StringToValue(string fieldType, string s)
        {
            if (!string.IsNullOrEmpty(s) && s.StartsWith("%js%"))
            {
                ...
                if (!((v is string)) || ((fieldType == null) || (fieldType == "String")))
                    return v;
                s = ((string)(v));
            }
            else
            ...
            if (fieldType != null)
                return TypeDescriptor.GetConverter(Controller.TypeMap[fieldType]).ConvertFromString(s);
            return s;
        }
        ...
        {
            ViewPage page = null;
            var cacheItem = new DataCacheItem(controller, request);
            if (cacheItem.HasValue)
                return ((ViewPage)(cacheItem.Value));
            SelectView(controller, view);
            request.AssignContext(controller, this._viewId, _config);
            page = new ViewPage(request);
        ...
        public virtual Stream GetDataControllerStream(string controller)
        {
            ...
            var sysControllersPath = ("sys/controllers%/" + (controller + ".xml"));
        ...
        public virtual string GetSurvey(string surveyName)
        {
            ...
            if (string.IsNullOrEmpty(survey))
                survey = ControllerConfigurationUtility.GetResourceText(string.Format("StefanTutorialDemo.Surveys.{0}.min.js", surveyName), string.Format("StefanTutorialDemo.{0}.min.js", surveyName), string.Format("StefanTutorialDemo.Surveys.{0}.js", surveyName), string.Format("StefanTutorialDemo.{0}.js", surveyName));
            if (string.IsNullOrEmpty(survey))
                throw new HttpException(404, "Not found.");
            if (string.IsNullOrEmpty(layout))
                layout = ControllerConfigurationUtility.GetResourceText(string.Format("StefanTutorialDemo.Surveys.{0}.html", surveyName), string.Format("StefanTutorialDemo.Surveys.{0}.htm", surveyName), string.Format("StefanTutorialDemo.{0}.html", surveyName), string.Format("StefanTutorialDemo.{0}.htm", surveyName));
            if (string.IsNullOrEmpty(rules))
                rules = ControllerConfigurationUtility.GetResourceText(string.Format("StefanTutorialDemo.Surveys.{0}.rules.min.js", surveyName), string.Format("StefanTutorialDemo.{0}.rules.min.js", surveyName), string.Format("StefanTutorialDemo.Surveys.{0}.rules.js", surveyName), string.Format("StefanTutorialDemo.{0}.rules.js", surveyName));
        ...
        protected virtual void EnsureSystemPageFields(PageRequest request, ViewPage page, DbCommand command)
        {
            if (page.Distinct)
            {
                ...
                while (i < page.Fields.Count)
                    if (page.Fields[i].IsPrimaryKey && ((request.FieldFilter == null) || !request.FieldFilter.Contains(page.Fields[i].Name)))
        ...
        protected virtual bool ValidateViewAccess(string controller, string view, string access)
        {
            if (!ApplicationServicesBase.AuthorizationIsSupported)
                return true;
            var context = HttpContext.Current;
            if ((AllowPublicAccess || FullAccessGranted(controller)) || (!string.IsNullOrEmpty(context.Request.Params["_validationKey"]) && (context.Request.Params["_validationKey"] == TextUtility.ToMD5Hash(ApplicationServices.ValidationKey))))
                return true;
