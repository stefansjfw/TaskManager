...
namespace StefanTutorialDemo.Data
{
    public partial class DataControllerBase
    {
        ...
        private string ProcessViewFilter(ViewPage page, DbCommand command, SelectClauseDictionary expressions)
        {
            ...
            var filter = Regex.Replace(_viewFilter, "/\\*Sql\\*/(?'Sql'[\\s\\S]+)/\\*Sql\\*/|(?'Parameter'(@|:)\\w+)|(?'Other'(\"|\'|\\[|`)\\s*\\w+)|(?\'Function\'\\$\\w+\\s*\\((?\'Arguments\'[\\S\\s]*?)\\))|(?\'Name\'\\w+)", DoReplaceKnownNames);
            return filter;
        }
                ...
                foreach (var filterExpression in page.Filter)
                {
                    ...
                    if (filterMatch.Success)
                    {
                        ...
                        while (valueMatch.Success)
                        {
                            ...
                            if ((field == null) && !fieldAlias.StartsWith("_"))
                            {
                                var fieldNav = Config.SelectSingleNode("/c:dataController/c:fields/c:field[@name='{0}']", fieldAlias);
                                if (fieldNav != null)
                                    field = new DataField(fieldNav, Config.Resolver);
                            }
                            if (((((field != null) && field.AllowQBE) || (operation == "~")) && (((page.DistinctValueFieldName != field.Name || (matchListCount > 0)) || (operation == "~")) || (page.AllowDistinctFieldInFilter || page.CustomFilteredBy(field.Name)))) || deepSearching)
                            {
                                ...
                                else
                                {
                                    if (operation == "~")
                                    {
                                        ...
                                        var textDateNumber = ("\\p{L}\\d" + Regex.Escape((currentCulture.DateTimeFormat.DateSeparator + (currentCulture.DateTimeFormat.TimeSeparator + currentCulture.NumberFormat.NumberDecimalSeparator))));
                                        ...
                                        var m = Regex.Match(paramValue, string.Format("\\s*(?'Token'((?'Quote'\")(?\'Value\'.+?)\")|((?'Quote'\\')(?'Value'.+?)\\')|(,|;|(^|\\s+)-)|(?'Value'[{0}]+))", textDateNumber));
                                                                            ...
                                                                            else
                                                                            {
                                                                                ...
                                                                                else
                                                                                {
                                                                                    ...
                                                                                    else
                                                                                    {
                                                                                        ...
                                                                                        {
                                                                                            if (!string.IsNullOrEmpty(tf.ItemsTargetController))
                                                                                                AppendManyToManyFilter(tf, page, command, "$contains$", pv.Trim('%'), sb);
                                                                                            else
                                    ...
                                    else
                                    {
                                        if (!string.IsNullOrEmpty(field.ItemsTargetController))
                                            AppendManyToManyFilter(field, page, command, operation, paramValue, sb);
                                        else
                                        {
        ...
        private void AppendManyToManyFilter(DataField field, ViewPage page, DbCommand command, string operation, string paramValue, StringBuilder sb)
        {
            ...
            foreach (var f in page.Fields)
                if (f.IsPrimaryKey)
                {
                    r.InnerJoinPrimaryKey = ("resultset__." + f.Name);
                    r.InnerJoinForeignKey = targetForeignKey1;
                    break;
                }
            var targetConfig = DataControllerBase.CreateConfigurationInstance(GetType(), field.ItemsTargetController);
            var operatorFieldItems = targetConfig.SelectSingleNode("/c:dataController/c:fields/c:field[@name='{0}']/c:items", targetForeignKey2);
            var operatorFieldName = targetForeignKey2;
            var aliasFieldNameNav = targetConfig.SelectSingleNode("/c:dataController/c:views/c:view//c:dataField[@fieldName='{0}' and @aliasFieldName]", targetForeignKey2);
            if (aliasFieldNameNav != null)
                operatorFieldName = aliasFieldNameNav.GetAttribute("aliasFieldName", string.Empty);
            ...
            if (operation == "$doesnotbeginwith$")
            {
                isNegative = true;
                operation = "$beginswith$";
            }
            if (operation == "$doesnotendwith$")
            {
                isNegative = true;
                operation = "$endswith$";
            }
            string staticFilterExpression = null;
            if (operation == "$isempty$")
            {
                isNegative = true;
                staticFilterExpression = "1=1";
            }
            if (operation == "$isnotempty$")
                staticFilterExpression = "1=1";
            var paramList = new List<string>();
            if (operation == "$in$")
                paramList.AddRange(Regex.Split(paramValue, "\\$(?:or|and)\\$"));
            else
            ...
            for (var i = 0; (i < paramList.Count); i++)
            {
                ...
                if (string.IsNullOrEmpty(staticFilterExpression))
                    r.Filter = new string[] {
                            string.Format("{0}:{1}{2}", operatorFieldName, operation, paramList[i])};
                controller.ConfigureSelectExistingCommand(r, command, sb);
                if (!string.IsNullOrEmpty(staticFilterExpression))
                    sb.Append(staticFilterExpression);
                sb.AppendLine(")");
            }
            if (isNegative)
                sb.AppendLine(")");
        ...
        protected virtual void AppendDeepFilter(string hint, ViewPage page, DbCommand command, StringBuilder sb, string filter)
        {
            ...
            for (var i = 0; (i < NegativeFilterOperations.Length); i++)
            {
                ...
                {
                    sb.Append("not ");
                    filter = (filter.Substring(0, (index + 1)) + (ReverseNegativeFilterOperations[i] + filter.Substring((index + (1 + negativeOperation.Length)))));
                    break;
                }
            }
            sb.Append("exists(");
        ...
        protected virtual void ConfigureSelectExistingCommand(PageRequest request, DbCommand command, StringBuilder sb)
        {
            ...
            while (parameterIndex >= 0)
            {
                ...
                var newParameterName = (_parameterMarker + ("cp" + command.Parameters.Count.ToString()));
            ...
            if (resultSetIndex < resultSetLastIndex)
                commandText = (commandText.Substring(0, (resultSetIndex + 9)) + ("2" + commandText.Substring((resultSetIndex + 9))));
        ...
        protected virtual void AppendExternalFilter(DbCommand command)
        {
            var externalFilterExpression = ((string)(HttpContext.Current.Items[(_config.ControllerName + "_FilterExpression")]));
            if (!string.IsNullOrEmpty(externalFilterExpression))
            {
                if (!string.IsNullOrEmpty(_viewFilter))
                    _viewFilter = (_viewFilter + "and");
                else
                    _viewFilter = string.Empty;
                _viewFilter = string.Format("{0}({1})", _viewFilter, externalFilterExpression);
                foreach (var p in ((Dictionary<string, object>)(HttpContext.Current.Items[(_config.ControllerName + "_FilterParameters")])))
                {
                    var parameter = command.CreateParameter();
                    parameter.ParameterName = p.Key.Replace("@", _parameterMarker);
                    AssignParameterValue(parameter, p.Value.GetType().Name, p.Value);
                    command.Parameters.Add(parameter);
                }
            }
        }
