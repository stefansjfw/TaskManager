...
namespace StefanTutorialDemo.Data
{
    public partial class DataControllerBase
    {
        ...
        public static Regex SqlSelectRegex1 = new Regex("/\\*<select>\\*/(?'Select'[\\S\\s]*)?/\\*</select>\\*[\\S\\s]*?/\\*<from>\\*/(?'From'[\\S\\s]*)?/\\*</from>\\*[\\S\\s]*?/\\*(<order-by>\\*/(?'OrderBy'[\\S\\s]*)?/\\*</order-by>\\*/)?", RegexOptions.IgnoreCase);
        ...
        public static Regex TableNameRegex = new Regex("^(?'Table'((\\[|\"|`)([\\w\\s]+)?(\\]|\"|`)|\\w+)(\\s*\\.\\s*((\\[|\"|`)([\\w\\s]+)?(\\]|\"|`)|\\w+))*(\\s*\\.\\s*((\\[|\"|`)([\\w\\s]+)?(\\]|\"|`)|\\w+))*)(\\s*(as|)\\s*(\\[|\"|`|)([\\w\\s]+)?(\\]|\"|`|))", RegexOptions.IgnoreCase);

        private SelectClauseDictionary _expressions;

        public static Regex ParamDetectionRegex = new Regex("(?:(\\W|^))(?'Parameter'(@|:)\\w+)");

        public static Regex SelectExpressionRegex = new Regex("\\s*(?'Expression'[\\S\\s]*?(\\([\\s\\S]*?\\)|(\\.((\"|\'|\\[|`)(?\'FieldName\'[\\S\\s]*?)(\"|'|\\]|`))|(\"|\'|\\[|`|)(?\'FieldName\'[\\w\\s]*?)(\"|'|\\]|)|)))((\\s+as\\s+|\\s+)(\"|\'|\\[|`|)(?\'Alias\'[\\S\\s]*?)|)(\"|'|\\]|`|)\\s*(,|$)", RegexOptions.IgnoreCase);
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if ((res == null) || (res == DefaultDataControllerStream))
                    res = ControllerConfigurationUtility.GetResourceStream(string.Format("StefanTutorialDemo.controllers.{0}.xml", controller), string.Format("StefanTutorialDemo.{0}.xml", controller));
                if (res == null)
                {
        ...
        public virtual void SelectView(string controller, string view)
        {
            ...
            if (!ValidateViewAccess(controller, _viewId, accessType))
                throw new Exception(string.Format("Not authorized to access private view '{0}' in data controller '{1}'. Set 'Access' property of the view to 'Public' or enable 'Idle User Detection' to automatically logout user after a period of inactivity.", _viewId, controller));
        }
        ...
        public virtual DataConnection CreateConnection(DataControllerBase controller, bool useTransaction)
        {
            ...
            if (useTransaction && SupportsTransaction())
            {
                ...
                if (!txn)
                    txn = (controller.Config.SelectSingleNode("/c:dataController/c:fields/c:field/c:items[@dataController and (@style='OneToOne' or @targetController!='')]") != null);
            }
        ...
        protected virtual bool ConfigureCommand(DbCommand command, ViewPage page, CommandConfigurationType commandConfiguration, FieldValue[] values)
        {
            ...
            if (command.CommandType == CommandType.Text)
            {
                ...
                if (statementMatch.Success)
                {
                    ...
                    if (commandIsCustom)
                    {
                        var customCommandText = EmbedFormulaExpressions(page, command.CommandText);
        ...
        protected virtual string EmbedFormulaExpressions(ViewPage page, string commandText)
        {
            var sb = new StringBuilder();
            foreach (var field in page.Fields)
                if (!string.IsNullOrEmpty(field.Formula))
                {
                    if (sb.Length > 0)
                        sb.AppendLine();
                    sb.AppendFormat("\t,({0}) {1}", field.Formula, field.Name);
                }
            if (sb.Length > 0)
            {
                var m = Regex.Match(commandText, "(\\s+)from(\\s+)");
                if (m.Success)
                    commandText = ((commandText.Substring(0, m.Index) + sb.ToString()) + commandText.Substring(m.Index));
            }
            return commandText;
        }
            ...
            foreach (var field in page.Fields)
                ...
                else
                {
                    if (generateFormula && field.Type != "DataView")
                    {
                        ...
                        {
                            var sqlExpr = string.Format("({0})", field.Name);
                            if (!string.IsNullOrEmpty(field.ItemsTargetController))
                                sqlExpr = "null";
                            expressions[field.ExpressionName()] = sqlExpr;
                        }
                    }
                }
        }
        ...
        private void ConfigureCommandForSelect(DbCommand command, ViewPage page, SelectClauseDictionary expressions, string fromClause, string whereClause, string orderByClause, CommandConfigurationType commandConfiguration)
        {
            ...
            else
            {
                ...
                else
                {
                    if ((commandConfiguration == CommandConfigurationType.Sync) && useLimit)
                        sb.Append("select * from (select @row_num := @row_num+1 row_number__,cte__.* from (select @row_num:=0) r,(");
                }
            ...
            if (commandConfiguration == CommandConfigurationType.Select)
            {
                ...
                if (useServerPaging)
                {
                    ...
                    {
                        sb.AppendFormat("where row_number__ > {0}PageRangeFirstRowNumber and row_number__ <= {0}PageRangeLastRowNumber order by row_number__", _parameterMarker);
                        var p = command.CreateParameter();
                        p.ParameterName = (_parameterMarker + "PageRangeFirstRowNumber");
                        p.Value = ((page.PageSize * page.PageIndex) + page.PageOffset);
                        if (preFetch)
                            p.Value = (((int)(p.Value)) - page.PageSize);
                        command.Parameters.Add(p);
                        var p2 = command.CreateParameter();
                        p2.ParameterName = (_parameterMarker + "PageRangeLastRowNumber");
                        p2.Value = ((page.PageSize * (page.PageIndex + 1)) + page.PageOffset);
                        if (preFetch)
            ...
            if (fromClause.Success)
            {
                ...
                if (expressions.ReferencedAliases != null)
                    foreach (var a in expressions.ReferencedAliases)
                    {
                        ...
                        {
                            var leftJoin = Regex.Match(fromClauseSql, (("left join .+ ('|\"|\\[|`)" + Regex.Escape(aliasName)) + "(\'|\"|\\|`]) on ('|\"|\\[)(?\'Alias\'\\w+)(\'|\"|\\|`])\\..+\\n"));
                            if (leftJoin.Success)
                            {
                                fromClauseSql = ((fromClauseSql.Substring(0, leftJoin.Index) + "inner") + (leftJoin.Value.Substring(4) + fromClauseSql.Substring((leftJoin.Index + leftJoin.Length))));
                ...
                sql = (sql.Substring(0, fromClause.Index) + (fromClauseSql + sql.Substring((fromClause.Index + fromClause.Length))));
            }
            return sql;
        }
        ...
        protected virtual bool ConfigureCTE(StringBuilder sb, ViewPage page, DbCommand command, SelectClauseDictionary expressions, bool performCount)
        {
            ...
            if (isOracle)
                sb.AppendFormat(",lpad(cast(row_number() over (partition by h1__.{0}{1}{2} order by h1__.{0}{3}{2} {4}) as varchar(5)), 5, '0') as {0}{5}{2}", _leftQuote, parentField.Name, _rightQuote, sortField.Name, sortOrder, hierarchyOrganization);
            else
                sb.AppendFormat(",cast(right('0000' + cast(row_number() over (partition by h1__.{0}{1}{2} order by h1__.{0}{3}{2} {4}) as varchar), 4) as varchar) as {0}{5}{2}", _leftQuote, parentField.Name, _rightQuote, sortField.Name, sortOrder, hierarchyOrganization);
            ...
            if (isOracle)
                sb.AppendFormat(",h__.{0}{5}{2} || '/' || lpad(cast(row_number() over (partition by h2__.{0}{1}{2} order by h2__.{0}{3}{2} {4}) as varchar(5)), 5, '0') as {0}{5}{2}", _leftQuote, parentField.Name, _rightQuote, sortField.Name, sortOrder, hierarchyOrganization);
            else
                sb.AppendFormat(",convert(varchar, h__.{0}{5}{2} + '/' + cast(right('0000' + cast(row_number() over (partition by h2__.{0}{1}{2} order by h2__.{0}{3}{2} {4}) as varchar), 4) as varchar)) as {0}{5}{2}", _leftQuote, parentField.Name, _rightQuote, sortField.Name, sortOrder, hierarchyOrganization);
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("),");
            sb.AppendFormat("ho__ as (select row_number() over (order by ({0}{1}{2})) as row_number__, h__.* from h__)", _leftQuote, hierarchyOrganization, _rightQuote);
            if (performCount)
        ...
        private void EnsurePageFields(ViewPage page, SelectClauseDictionary expressions)
        {
            ...
            var dynamicConfigIterator = _config.Select("/c:dataController/c:fields/c:field[c:configuration!='']/c:configuration|/c:dataController/c:fields/c:field/c:items[@copy!='']/@copy");
                ...
                if (fieldIterator.MoveNext())
                {
                    ...
                    if (!string.IsNullOrEmpty(search))
                    {
                        ...
                        if (ApplicationServices.IsTouchClient)
                            field.Tag = (field.Tag + (" search-mode-" + searchMode.ToString().ToLower()));
                        else
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
                t = typeof(StefanTutorialDemo.Data.Controller);
            var res = ControllerConfigurationUtility.GetResourceStream(string.Format("StefanTutorialDemo.Reports.{0}", name), string.Format("StefanTutorialDemo.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                var config = StefanTutorialDemo.Data.Controller.CreateConfigurationInstance(t, controller);
