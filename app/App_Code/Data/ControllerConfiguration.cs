...
namespace StefanTutorialDemo.Data
{
    public class ControllerConfiguration
    {
        ...
        protected virtual void Initialize(XPathNavigator navigator)
        {
            ...
            {
                var t = ApplicationServices.StringToType("StefanTutorialDemo.Rules.SharedBusinessRules");
                if (t != null)
        ...
        public virtual void Complete()
        {
            ...
            if (string.IsNullOrEmpty(_connectionStringName))
                _connectionStringName = "StefanTutorialDemo";
            ...
            while (ruleIterator.MoveNext())
            {
                ...
                rule.Result = string.Format("<id>{0}</id><command>{1}</command><argument>{2}</argument><view>{3}</view><phase>{4}</phase><js>{5}</js>", ruleNav.GetAttribute("id", string.Empty), ruleNav.GetAttribute("commandName", string.Empty), ruleNav.GetAttribute("commandArgument", string.Empty), ruleNav.GetAttribute("view", string.Empty), ruleNav.GetAttribute("phase", string.Empty), ruleNav.Value);
        ...
        public virtual ControllerConfiguration EnsureVitalElements()
        {
            ...
            if (viewsNode != null)
            {
                var sb = new StringBuilder("<view id=\"view1\" type=\"Form\" label=\"Form\"><categories><category id=\"c1\" flow=\"NewColumn\"><dataFields>");
...
<action id=""a5"" commandName=""Edit"" causesValidation=""true"" />
        ...
        public XPathNavigator SelectSingleNode(string selector, params object[] args)
        {
            return _navigator.SelectSingleNode(string.Format(selector, args), _resolver);
        }

        public XPathNodeIterator Select(string selector, params object[] args)
        {
            return _navigator.Select(string.Format(selector, args), _resolver);
        }

        public object Evaluate(string selector, params object[] args)
        {
        ...
        public string LoadLayout(string view)
        {
            ...
            while (tryLoad)
            {
                ...
                {
                    var stream = GetType().Assembly.GetManifestResourceStream(string.Format("StefanTutorialDemo.Views.{0}.{1}.html", this.ControllerName, view));
                    if (stream != null)
                    {
        ...
        public string ToJson()
        {
            ...
            json = (json.Substring(0, eof.Index) + (",\"expressions\":" + (expressions + eof.Value)));
            return json;
        }
    }
    ...
    public class ControllerConfigurationUtility
    {
        ...
        public static string[] GetDataControllerList()
        {
            var list = new List<string>();
            foreach (var r in _assemblyResources)
            {
                var m = Regex.Match(r.Value, "^StefanTutorialDemo\\.(controllers\\.)?(.+?)\\.xml$", RegexOptions.IgnoreCase);
                if (m.Success)
                {
                    var controllerName = m.Groups[2].Value;
            ...
            if (list.Count == 0)
            {
                var controllerCollectionPath = Path.Combine(Path.Combine(HttpRuntime.AppDomainAppPath, "Controllers"));
                if (Directory.Exists(controllerCollectionPath))
                    foreach (var fileName in Directory.GetFiles(controllerCollectionPath, "*.xml"))
                    {
