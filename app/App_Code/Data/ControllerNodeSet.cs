...
namespace StefanTutorialDemo.Data
{
    public class ControllerNodeSet
    {
        ...
        private static Regex _elementNameRegex = new Regex("(?'String'\"([\\s\\S]+?)\")|(?'String'\\'([\\s\\S]+?)\\')|(?'AttrOrVar'(@|\\$)\\w+)|(?'Function'\\w+\\s*\\()|(?'Element'(?'Axis'[\\w-]+::)?(?'Namespace'\\w+\\:)?(([\\w-]+::\\*)|(?'Name'[a-z]\\w*)))", RegexOptions.IgnoreCase);
        ...
        public ControllerNodeSet Add(string selector, params object[] args)
        {
        ...
        public ControllerNodeSet Select(string selector, params object[] args)
        {
        ...
        private ControllerNodeSet InternalSelect(bool add, string selector, params object[] args)
        {
            _argIndex = 0;
        ...
        /// <example>field[@name=$name]</example>
        /// <returns>The collection of child nodes after they were appended to the nodes in the original node set.</returns>
        public ControllerNodeSet AppendTo(string selector, params object[] args)
        {
        ...
        private ControllerNodeSet SelectInContext(string contextNode, string selector, params object[] args)
        {
            foreach (var node in _nodes)
        ...
        public ControllerNodeSet SelectItems(params object[] values)
        {
            if (values.Length == 0)
                return Select("item");
        ...
        public ControllerNodeSet VisibleWhen(string clientScript, params object[] args)
        {
        ...
        public ControllerNodeSet ReadOnlyWhen(string clientScript, params object[] args)
        {
