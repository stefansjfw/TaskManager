...
namespace StefanTutorialDemo.Data
{
    ...
    public class TransactionManagerBase
    {
        ...
        {
            var list = new List<ActionArgs>();
            foreach (var entry in log)
            {
                var executeArgs = entry["args"].ToObject<ActionArgs>();
                var controller = ((string)(entry["controller"]));
                var view = ((string)(entry["view"]));
                if ((controller == executeArgs.Controller) && (view == executeArgs.View))
                    list.Add(executeArgs);
            }
            return CommitAll(list);
        }

        public virtual CommitResult CommitAll(List<ActionArgs> log)
        {
            ...
            try
            {
                if (log.Count > 0)
                {
                    using (var tx = new DataTransaction(LoadConfig(log[0].Controller).ConnectionStringName))
                    {
                        var index = -1;
                        var sequence = -1;
                        ...
                        {
                            ActionArgs.Forget();
                            var executeArgs = log[i];
                            var entry = log[i];
                            var controller = executeArgs.Controller;
                            var view = executeArgs.View;
                            ...
                            if (executeResult.Errors.Count > 0)
                            {
                                ...
                                _commitResult.RowNotFound = executeResult.RowNotFound;
                                break;
                            }
                            else
