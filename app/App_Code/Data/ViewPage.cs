...
namespace StefanTutorialDemo.Data
{
    public class ViewPage
    {
        ...
        public void ResetSkipCount(bool preFetch)
        {
            if (preFetch)
            {
                _skipCount = ((_pageIndex - 1) * _pageSize);
                                ...
                                if (contextValues != null)
                                    foreach (var cv in contextValues)
                                        if (cv.Name == m.Groups[2].Value)
                                        {
                                            if (cv.Value == null)
                                                return true;
                                            contextFilter.Add(string.Format("{0}:={1}", m.Groups[1].Value, cv.Value));
                                            break;
                                        }
                            }
                            m = m.NextMatch();
                ...
                if (IncludeMetadata("views"))
                {
                    ...
                    while (viewIterator.MoveNext())
                    {
                        ...
                        if (v.Id == this.View)
                        {
                            ...
                            if ((v.Type == "Grid") && (configuration.SelectSingleNode("/c:dataController/c:businessRules/c:rule[(@type='Sql' or @type='Code') and @commandName = 'New']") != null))
            ...
            if (_filter != null)
                foreach (var s in _filter)
                {
                    ...
                    if (m.Success == m.Groups["Alias"].Value.Contains(","))
                        return true;
                    if (!s.StartsWith("_match_"))
                    {
                        var filterInfo = s.Split(':');
                        if (filterInfo.Length > 1)
                        {
                            if (filterInfo[0] == "_quickfind_")
                            {
                                // require a resulset if there are many-to-many feilds
                                foreach (var f in Fields)
                                    if (!string.IsNullOrEmpty(f.ItemsTargetController))
                                        return true;
                            }
                            else
                            {
                                var field = FindField(filterInfo[0]);
                                if ((field != null) && !string.IsNullOrEmpty(field.ItemsTargetController))
                                    return true;
                            }
                        }
                    }
                            ...
                            for (var i = 0; (i < additionalProperties.Count); i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            // expand auto date
                                            var newPivotID = (int.MaxValue - (pivotID * 6));
                                            var newTag = (string.Format("pivotgroup{0}-", pivotID) + (Regex.Replace(tag, "pivot(\\d+?)-|row(\\d+?)|date|all", string.Empty) + "-pivot"));
                                            field.Tag = ((newTag + newPivotID.ToString()) + "-row1-year ");
                                            AddPivotField(field);
                                            newPivotID++;
                                            // quarter
                                            field.Tag = ((newTag + newPivotID.ToString()) + "-row1-year ");
                                            field.Tag = (field.Tag + ((newTag + newPivotID.ToString()) + "-row2-quarter"));
                                            AddPivotField(field);
                                            newPivotID++;
                                            // month
                                            field.Tag = ((newTag + newPivotID.ToString()) + "-row1-year ");
                                            field.Tag = (field.Tag + ((newTag + newPivotID.ToString()) + "-row2-month"));
                                            AddPivotField(field);
                                            newPivotID++;
                                            // week of year
                                            field.Tag = ((newTag + newPivotID.ToString()) + "-row1-year ");
                                            field.Tag = (field.Tag + ((newTag + newPivotID.ToString()) + "-row2-month "));
                                            field.Tag = (field.Tag + ((newTag + newPivotID.ToString()) + "-row3-weekofyear"));
                                            AddPivotField(field);
                                            newPivotID++;
                                            // day
                                            field.Tag = ((newTag + newPivotID.ToString()) + "-row1-year ");
                                            field.Tag = (field.Tag + ((newTag + newPivotID.ToString()) + "-row2-month "));
                                            field.Tag = (field.Tag + ((newTag + newPivotID.ToString()) + "-row3-day"));
                                            AddPivotField(field);
                                            newPivotID++;
                                            fi.Bucket = "year";
                                        }
    ...
    public class PivotTable
    {
                ...
                if (string.IsNullOrEmpty(_valuesName))
                {
                    ...
                    foreach (var info in ValueFields.Values)
                    {
                        if (first)
                            ...
                            sb.Append(", ");
                        sb.Append((("$" + info.Mode.Substring(0, 1).ToUpper()) + info.Mode.Substring(1)));
                        sb.Append("Of ");
            ...
            if (fieldInfo.ExpandBuckets)
            {
                ...
                else
                {
                    ...
                    if (string.IsNullOrEmpty(field))
                    {
                        ...
                        var fieldNav = config.SelectSingleNode("/c:dataController/c:views/c:view[@type!='Form'][1]/c:dataFields/c:dataField[@fieldName!=/c:dataController/c:fields/c:field[@isPrimaryKey]/@name][1]/@fieldName");
                        if (fieldNav != null)
        ...
        static int GetWeekNumberOfMonth(System.DateTime d)
        {
            ...
            if (firstDay == 0)
                firstDay = 7;
            var d2 = ((firstDay + (d.Day - 1)) / Convert.ToDouble(7));
