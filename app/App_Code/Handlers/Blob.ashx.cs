...
namespace MyCompany.Handlers
{
    ...
    public class BlobHandlerInfo
    {
        ...
        private List<FieldValue> CreateActionValues(Stream stream, string contentType, string fileName, int contentLength)
        {
            ...
            if (stream != null)
            {
                ...
                if (contentTypeFieldNav == null)
                    contentTypeFieldNav = config.SelectSingleNode("/c:dataController/c:fields/c:field[@name='{0}ContentType' or @name='{0}CONTENTTYPE']", ControllerFieldName);
                if (contentTypeFieldNav == null)
        ...
        private bool ProcessUploadViaBusinessRule(BlobAdapter ba)
        {
            ...
            Blob.CreateDataController().Execute(DataController, UploadDownloadViewName, args);
            r.RaiseExceptionIfErrors();
            return canceled;
        }
    ...
    public class Blob : GenericHandlerBase, IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public static int ThumbnailCacheTimeout = 10;
        ...
        public static bool StreamOut
        {
            get
            {
                return Convert.ToBoolean(HttpContext.Current.Items["BlobHandlerInfo_StreamOut"]);
            }
        }
        ...
        void IHttpHandler.ProcessRequest(HttpContext context)
        {
            ...
            if (handler.Mode == BlobMode.Upload)
            {
                ...
                else
                {
                    if (!success)
                    {
                        ...
                        context.Response.Write(handler.Error);
            ...
            else
            {
                if (Blob.DirectAccessMode)
                {
                    ...
                    if (Blob.StreamOut)
                    {
                        ApplicationServicesBase.SendETagIfNoneModified(data);
                        stream.Position = 0;
        ...
        public static void Send(string blobHandler, object keyValue)
        {
            var context = HttpContext.Current;
            context.Items["BlobHandlerInfo_StreamOut"] = true;
            Read(blobHandler, keyValue);
            context.Items.Remove("BlobHandlerInfo_StreamOut");
        }
        ...
        public static void CopyToOutput(HttpContext context, Stream stream, BlobHandlerInfo handler)
        {
            var offset = 0;
            ...
            if (handler.AllowCaching)
            {
                ...
                if ((img != null) && (handler.Mode == BlobMode.Original))
                {
                    ...
                    thumbHeight = Convert.ToInt32((img.Height * (thumbWidth / Convert.ToDouble(img.Width))));
                ...
                if (img != null)
                {
                    if (!handler.MaxWidth.HasValue)
                    {
                        ...
                        else
                        {
                            if (img.Width > img.Height)
                            {
                                ...
                                r.Width = Convert.ToInt32((Convert.ToDouble(r.Height) * (Convert.ToDouble(img.Width) / Convert.ToDouble(img.Height))));
                            }
                            else
                            {
                                if (img.Height > img.Width)
                                {
                                    ...
                                    r.Height = Convert.ToInt32((Convert.ToDouble(r.Width) * (Convert.ToDouble(img.Height) / Convert.ToDouble(img.Width))));
                                }
                                else
                                {
                    ...
                    r.Location = new Point(((thumbnail.Width - r.Width) / 2), ((thumbnail.Height - r.Height) / 2));
                ...
                else
                {
                    ...
                    context.Response.Cache.SetMaxAge(TimeSpan.FromMinutes(Blob.ThumbnailCacheTimeout));
                    context.Response.Cache.SetProxyMaxAge(TimeSpan.FromMinutes(Blob.ThumbnailCacheTimeout));
                }
        ...
        if (parent && parent.window.Web) {
            ...
            if (uploadSuccess)
            {
                if (HttpContext.Current.Request.Files[0].ContentLength > 0)
                    message = string.Format(Localizer.Replace("BlobUploded", "<b>Confirmation:</b> {0} has been uploaded successfully. <b>It may take up to {1} minutes for the thumbnail to reflect the uploaded content.</b>"), handler.Text.ToLower(), Blob.ThumbnailCacheTimeout);
                else
