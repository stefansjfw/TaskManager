...
namespace StefanTutorialDemo.Handlers
{
    ...
    public class SiteBase : StefanTutorialDemo.Web.PageBase
    {
        ...
        public static string Copyright
        {
            get
            {
                return "&copy; 2024 StefanTutorialDemo. ^Copyright^All rights reserved.^Copyright^";
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            var isRESTfulRequest = RESTfulResource.IsRequested;
            if (!isRESTfulRequest && (User.Identity.IsAuthenticated && !ApplicationServices.AllowUI(User.Identity.Name)))
            {
                FormsAuthentication.SignOut();
                app.RedirectToLoginPage();
                Response.End();
            }
            if (isRESTfulRequest || (Request.Path.StartsWith((ResolveUrl(AquariumExtenderBase.DefaultServicePath) + "/"), StringComparison.CurrentCultureIgnoreCase) || Request.Path.StartsWith((ResolveUrl(AquariumExtenderBase.AppServicePath) + "/"), StringComparison.CurrentCultureIgnoreCase)))
                ApplicationServicesBase.HandleServiceRequest(Context);
            if (app.HandleWebAppRequest())
                Response.End();
            ...
            var appName = new LiteralControl(string.Format("<meta name=\"application-name\" content=\"{0}\">", HttpUtility.HtmlAttributeEncode(app.DisplayName)));
            Header.Controls.Add(appName);
            if (ApplicationServicesBase.IsInstallable && (ApplicationServices.AppState == null))
            {
                Header.Controls.Add(new LiteralControl(string.Format("<link rel=\"manifest\" href=\"{0}\">", AppResourceManager.ToResourceName("~/manifest.json"))));
                Header.Controls.Add(new LiteralControl("<link rel=\"apple-touch-icon\" href=\"~/images/icons/app-192x192.png\">"));
                Header.Controls.Add(new LiteralControl(string.Format("<meta name=\"theme-color\" media=\"(prefers-color-scheme: light)\" content=\"{0}\">", ApplicationServicesBase.SettingsProperty("client.manifest.theme_color", "#fff"))));
                Header.Controls.Add(new LiteralControl(string.Format("<meta name=\"theme-color\" media=\"(prefers-color-scheme: dark)\" content=\"{0}\">", ApplicationServicesBase.SettingsProperty("client.manifest.dark_mode_theme_color", "#000"))));
            }
                        ...
                        if (string.IsNullOrEmpty(controlExtension))
                        {
                            ...
                            else
                            {
                                if (ApplicationServices.IsSiteContentEnabled)
                                {
                                    ...
                                    siteControlText = app.ReadSiteContentString(("sys/" + relativeControlPath));
                                }
            ...
            else
            {
                if (_isTouchUI)
                {
                    ...
                    if (Context.Request.Path.EndsWith("/pages/offline"))
                        notFoundError = "<div data-app-role=\"sitemap\"></div>";
                    _pageContent.Text = string.Format("<div id=\"PageContent\" style=\"display:none\"><div data-app-role=\"page\">{0}</div></div>", notFoundError);
            ...
            if (_isTouchUI)
            {
                if (_pageFooterContent != null)
                    _pageFooterContent.Text = (("<footer style=\"display:none\"><small>" + Copyright) + "</small></footer>");
            }
            else
            {
                if (contentInfo.TryGetValue("About", out s))
                {
                    if (_pageSideBarContent != null)
                        _pageSideBarContent.Text = string.Format("<div class=\"TaskBox About\"><div class=\"Inner\"><div class=\"Header\">About</div><div class=\"Value\">{0}</div></div></div>", s);
                }
            }
            ...
            if (!_isTouchUI)
            {
                ...
                classAttr = ((classAttr + " ") + (Regex.Replace(Request.Path.ToLower(), "\\W", "_").Substring(1) + "_html"));
            }
            else
            {
                if (_summaryDisabled)
            ...
            if (prefetch)
                for (var i = 0; (i < dataViews.Count); i++)
                {
                    ...
                    if (attrs.TryGetValue("data-roles", out roles) && !new ControllerUtilities().UserIsInRole(roles.Split(',')))
                        return output;
                    tags = (tags + (" " + viewNav.GetAttribute("tags", string.Empty)));
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            var htmlMatch = Regex.Match(html, "<html(?'HtmlAttr'[\\S\\s]*?)>\\s*<head(?'HeadAttr'[\\S\\s]*?)>\\s*(?'Head'[\\S\\s]*?)\\s*</head>\\s*<body(?'BodyAttr'[\\S\\s]*?)>\\s*(?'Body'[\\S\\s]*?)\\s*</body>\\s*</html>\\s*");
            ...
            var themePath = Server.MapPath("~/App_Themes/StefanTutorialDemo");
            if (Directory.Exists(themePath))
                foreach (var stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    ...
                    if (!fileName.Equals("_Theme_Aquarium.css"))
                    {
                        ...
                        {
                            Href = ("~/App_Themes/StefanTutorialDemo/" + fileName)
                        };
            ...
            var placeholderMatch = Regex.Match(body, "<div\\s+data-role\\s*=\\s*\"placeholder\"(?'Attributes'[\\s\\S]+?)>\\s*(?'DefaultContent'[\\s\\S]*?)\\s*</div>");
            var startPos = 0;
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            content = Regex.Replace(content, "<div\\s+data-role\\s*=\"placeholder\"\\s+(?'Attributes'[\\s\\S]+?)>(?'DefaultContent'[\\s\\S]*?)</div>", DoReplacePlaceholder);
            if (ApplicationServices.AppState == null)
            {
                var rawText = ((string)(Context.Items["ui-framework-none.text"]));
                if (rawText != null)
                {
                    var scripts = Regex.Matches(content, "<script[\\s\\S]+?</script>");
                    if (scripts.Count > 0)
                    {
                        var lastScript = scripts[(scripts.Count - 1)];
                        if (lastScript.Value.Contains("Web.Menu.Nodes.Menu1"))
                            content = (rawText.Replace("</body>", lastScript.Value.Replace("Web.Menu.Nodes.Menu1", "var __menu")) + "</body>");
                        else
                            content = rawText;
                    }
                }
            ...
            ApplicationServicesBase.CompressOutput(Context, content);
        }
