    ...
    public class MyProfileBusinessRulesBase : BusinessRules
    {
        ...
        protected override void VirtualizeController(string controllerName)
        {
            ...
            if (OAuthProviders.Count > 0)
            {
                ...
                var appIdentityIssuer = string.Empty;
                if (items.Count > 0)
                {
                    var autoLoginWith = string.Empty;
                    ...
                    foreach (var file in OAuthProviders)
                    {
                        ...
                        try
                        {
                            var providerDef = TextUtility.ParseYamlOrJson(file.Text);
                            if (Convert.ToString(true).Equals(Convert.ToString(providerDef["Auto Login"]), StringComparison.OrdinalIgnoreCase))
                                autoLoginWith = file.Name;
                            if (file.Name == "appidentity")
                            {
                                var appUrl = new Uri(ApplicationServicesBase.ResolveClientUrl("~/")).ToString();
                                var providerUri = Convert.ToString(providerDef["Client Uri"]);
                                var localProviderUri = Convert.ToString(providerDef["Local Client Uri"]);
                                if ((appUrl == providerUri) || (appUrl == localProviderUri))
                                    addProvider = false;
                                else
                                {
                                    // the consumer must remove all OAuth options if the App Identity provider is available
                                    supportedProviders.Clear();
                                    supportedProviders.Add("appidentity");
                                    appIdentityIssuer = ((string)(Context.Cache["AppIdentityIssuer"]));
                                    if (string.IsNullOrEmpty(appIdentityIssuer))
                                    {
                                        appIdentityIssuer = "App Identity";
                                        try
                                        {
                                            if (!providerUri.EndsWith("/"))
                                                providerUri = (providerUri + "/");
                                            var request = WebRequest.Create(new Uri(new Uri(providerUri), "oauth2/v2"));
                                            using (var response = request.GetResponse())
                                            {
                                                using (var sr = new StreamReader(response.GetResponseStream()))
                                                {
                                                    var oauthApi = TextUtility.ParseYamlOrJson(sr.ReadToEnd());
                                                    appIdentityIssuer = Convert.ToString(oauthApi["issuer"]);
                                                }
                                            }
                                        }
                                        catch (Exception)
                                        ...
                                        Context.Cache["AppIdentityIssuer"] = appIdentityIssuer;
                                    }
                                    break;
                                }
                            }
                    ...
                    foreach (var item in items)
                    {
                        ...
                        if ((provider == "other") && string.IsNullOrEmpty(autoLoginWith))
                        {
                        ...
                        else
                        {
                            ...
                            {
                                if ((provider == "appidentity") && (otherItemText != null))
                                    otherItemText.SetValue(appIdentityIssuer);
                            }
                        }
                    }
                }
