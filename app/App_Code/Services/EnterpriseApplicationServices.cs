...
namespace MyCompany.Services
{
    ...
    public partial class AutoFillAddress : AutoFillAddressBase
    {
        ...
        static AutoFillAddress()
        {
            ...
            Formats["address1_DE"] = "{route}{when /./ in street_number then  }{street_number}{when /./ in subpremise then , }{when /./ in subpremise then subpremise}";
            ...
            Formats["address1"] = "{route}{when /./ in street_number then , }{street_number}{when /./ in subpremise then , }{when /./ in subpremise then subpremise}";
            Formats["city"] = "{postal_town,sublocality,neighborhood,locality}";
            Formats["postalcode"] = "{postal_code}{when /./ in postal_code_suffix then -}{when /./ in postal_code_suffix then postal_code_suffix}";
    ...
    public class AutoFillAddressBase : AutoFill
    {
        ...
        protected override JToken Process(BusinessRulesBase rules, JObject autofill)
        {
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(ApplicationServicesBase.Settings("server.geocoding.google.key"))))
            {
            ...
            try
            {
                ConfirmResult(rules, outputAddressList, autofill);
            }
            catch (Exception)
            {
                // do nothing
        ...
        protected virtual string Format(string type, JObject components, string defaultValue)
        {
            ...
            if (!Formats.TryGetValue((type + ("_" + country)), out s))
            {
            ...
            while (true)
            {
                ...
                if (m.Success)
                {
                    ...
                    if (v == null)
                        v = string.Empty;
                    s = (s.Substring(0, m.Index) + (v + s.Substring((m.Index + m.Length))));
                }
                else
                    break;
            }
        ...
        protected virtual void ConfirmResult(BusinessRulesBase rules, JArray addresses, JObject autofill)
        {
            // confirm with USPS
            var userName = ((string)(ApplicationServicesBase.Settings("server.geocoding.usps.userName")));
            if (!string.IsNullOrEmpty(userName))
            {
                var removeIfNoMatch = false;
                if (addresses.Count == 0)
                {
                    removeIfNoMatch = true;
                    autofill["state"] = autofill["region"];
                    autofill["components"] = new JObject(new JProperty("country", "US"));
                    addresses.Add(autofill);
                }
                foreach (JToken address in addresses)
                    if (((string)(address["components"]["country"])) == "US")
                    {
                        ...
                        var canSearch = (!((string.IsNullOrEmpty(Convert.ToString(address["state"])) || string.IsNullOrEmpty(Convert.ToString(address["city"])))) || !string.IsNullOrEmpty(Convert.ToString(address["postalcode"])));
                        if (!string.IsNullOrEmpty(address1) && canSearch)
                        {
                            var uspsRequest = new StringBuilder();
                            var settings = new XmlWriterSettings()
                            {
                                OmitXmlDeclaration = true
                            };
                            var writer = XmlWriter.Create(new StringWriter(uspsRequest), settings);
                            writer.WriteStartElement("AddressValidateRequest");
                            writer.WriteAttributeString("USERID", userName);
                            writer.WriteElementString("Revision", "1");
                            writer.WriteStartElement("Address");
                            writer.WriteAttributeString("ID", "0");
                            writer.WriteElementString("Address1", Convert.ToString(address["address2"]));
                            writer.WriteElementString("Address2", address1);
                            writer.WriteElementString("City", Convert.ToString(address["city"]));
                            writer.WriteElementString("State", Convert.ToString(address["state"]));
                            writer.WriteElementString("Zip5", Convert.ToString(address["postalcode"]));
                            writer.WriteStartElement("Zip4");
                            ...
                            using (var client = new WebClient())
                            {
                                var uspsResponseText = client.DownloadString(("https://secure.shippingapis.com/ShippingAPI.dll?API=Verify&XML=" + HttpUtility.UrlEncode(uspsRequest.ToString())));
                                var uspsResponse = new XPathDocument(new StringReader(uspsResponseText)).CreateNavigator().SelectSingleNode("/AddressValidateResponse/Address");
                                if ((uspsResponse != null) && (uspsResponse.SelectSingleNode("Error") == null))
                                {
                                    address["address1"] = uspsResponse.SelectSingleNode("Address2").Value;
                                    var validatedAddress1 = uspsResponse.SelectSingleNode("Address1");
                                    if (validatedAddress1 != null)
                                        address["address2"] = validatedAddress1.Value;
                                    else
                                        address["address2"] = null;
                                    address["city"] = uspsResponse.SelectSingleNode("City").Value;
                                    address["region"] = uspsResponse.SelectSingleNode("State").Value;
                                    address["postalcode"] = string.Format("{0}-{1}", uspsResponse.SelectSingleNode("Zip5").Value, uspsResponse.SelectSingleNode("Zip4").Value);
                                    address["components"]["postalcode"] = uspsResponse.SelectSingleNode("Zip5").Value;
                                    address["components"]["postalcodesuffix"] = uspsResponse.SelectSingleNode("Zip4").Value;
                                    address["country"] = Convert.ToString(address["country"]).ToUpper();
                                    var address2 = Convert.ToString(address["address2"]);
                                    if (address2.Length > 0)
                                        address2 = (" " + address2);
                                    address["name"] = string.Format("{0}{1}, {2}, {3} {4}", address["address1"], address2, address["city"], address["region"], address["postalcode"]);
                                    address["country"] = "USA";
                                    removeIfNoMatch = false;
                                }
                            }
                        }
                    }
    ...
    public class AutoFillMapBase : AutoFill
    {
        ...
        protected virtual string CreateRequestUrl(BusinessRulesBase rules, JObject autofill)
        {
            ...
            return string.Format("https://maps.googleapis.com/maps/api/staticmap?size={0}&scale={1}&maptype={2}&zoom={3}&markers={4}&key={5}", ToSize(rules, autofill), ToScale(rules, autofill), ToMapType(rules, autofill), ToZoom(rules, autofill), ToMarkers(rules, autofill), ApplicationServicesBase.Settings("server.geocoding.google.key"));
        }
