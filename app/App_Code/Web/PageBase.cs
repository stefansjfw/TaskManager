...
namespace StefanTutorialDemo.Web
{
    ...
    public class PageBaseCore : System.Web.UI.Page
    {
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            tempWriter.Flush();
            tempWriter.Close();
            var page = StefanTutorialDemo.Data.Localizer.Replace("Pages", Path.GetFileName(Request.PhysicalPath), sb.ToString());
                ...
                if (page.Contains("data-content-framework=\"bootstrap\""))
                {
                    ...
                    else
                    {
                        if (ApplicationServicesBase.IsTouchClient)
                            page = Regex.Replace(page, "(<link\\s+href=\"[.\\w\\/]+?touch\\-theme\\..+?\".+?/>)", (("<link href=\"" + ResolveClientUrl(("~/css/sys/bootstrap.css?" + ApplicationServices.Version))) + "\" type=\"text/css\" rel=\"stylesheet\" />$1"));
                    }
                    if (AquariumExtenderBase.EnableCombinedScript && ApplicationServicesBase.IsTouchClient)
    ...
    public class ControlBaseCore : System.Web.UI.UserControl
    {
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            tempWriter.Flush();
            tempWriter.Close();
            writer.Write(StefanTutorialDemo.Data.Localizer.Replace("Pages", Path.GetFileName(Request.PhysicalPath), sb.ToString()));
        }
        ...
        public static System.Web.UI.Control LoadPageControl(System.Web.UI.Control placeholder, string pageName, bool developmentMode)
        {
            try
            {
                ...
                var basePath = "~";
                if (!developmentMode)
                    basePath = "~/DesktopModules/StefanTutorialDemo";
                var controlPath = string.Format("{0}/Pages/{1}.ascx", basePath, pageName);
                var c = page.LoadControl(controlPath);
                if (c != null)
                {
                    placeholder.Controls.Clear();
                    placeholder.Controls.Add(new LiteralControl("<table style=\"width:100%\" id=\"PageBody\" class=\"Hosted\"><tr><td valign=\"top\" id=\"PageContent\">"));
