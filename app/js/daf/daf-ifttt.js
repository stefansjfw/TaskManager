...
(function () {
    ...
    app = {
        ...
        notify: function (options, duration) {
            ...
            if (typeof options != 'object')
                options = { text: options, duration: duration };
        ...
        inc: function (options, barcode) {
            ...
            if (field) {
                ...
                return app.val({ identifier: options }, v);
            }
            else
    ...
    function kioskInit() {
        ...
        if ($settings('barcodeReader.enabled') !== false)
            $span('app-btn-scan ui-btn ui-btn-icon-notext ui-btn-left"').appendTo(_kiosk).attr('data-title', resourcesMobile.Scan);
        buttonAnchor = $span('app-btn-context ui-btn ui-btn-icon-notext ui-btn-left"').appendTo(_kiosk).attr('data-title', resourcesMobile.More);
        ...
        if (args.length && uiAutomationSettings('kiosk.enabled') !== false) {
            ...
            if (typeof show == 'boolean') {
                if (method) {
                    ...
                    _touch.resetUI();
                    $('.ui-page-active .app-wrapper').trigger('focus');
        ...
        while (execState.index < ifttt.length) {
            ...
            if (activity.background || id && activity.id === id) {
                try {
                    ...
                    var activityPromise = activity.execute(app, execState.context)
                        .done(iftttDone)
                        .fail(function (err) {
                            if (execState) {
                                var pendingError = execState.error || '';
                                if (err) {
                                    if (pendingError)
                                        pendingError += ' ';
                                    execState.error = pendingError + err.message;
                                    if (location.hostname === 'localhost')
                                        execState.error += '\n\n' + err.stack;
                                    exit(false);
                                }
                            //    else if (!execState.ifRejected && executeResult.ifResult != null)
                            //        exit(false);
                            }
                        });
