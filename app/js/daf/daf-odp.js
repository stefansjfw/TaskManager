...
(function () {
            ...
            switch (method) {
                    ...
                    if (odpFile)
                        ...
                        if (image) {
                            ...
                            (_odp.offline('toDataUrl', optionsSource) || _odp.toDataUrl(optionsSource)).done(function (url) {
                                blobChangeLog = blobChangeLog || _app.userVar('blobChangeLog');
                                if (blobChangeLog) {
                                    var timestamp = blobChangeLog[onDemandHandler + key];
                                    if (timestamp)
                                        url += '&t=' + timestamp;
                                    removeExpiredBlobChangeLogTimestamps();
                                }
                ...
                case 'url':
                    ...
                    if (url.match(/^data\:/))
                        ...
                        odpFile = toOdpFile(url, options.dataView);
                        if (odpFile)
                ...
                case 'download':
                    ...
                    if (odpFile)
                        ...
                        result = _odp.offline('toDataUrl', { url: url, blob: true });
                        if (result)
        ...
        uploadFile: function (options) {
            ...
            if (!result) {
                result = _app.uploadFileAjax({
                    url: String.format('~/blob.ashx?{0}=u|{1}&_v={2}', handler, options.key, options.apiVer),
                    files: options.file || options.files,
                    progress: options.progress
                });
            }
            blobChangeLog = blobChangeLog || _app.userVar('blobCache') || {};
            blobChangeLog[handler + options.key] = new Date().getTime();
            _app.userVar('blobChangeLog', blobChangeLog);
            return result;
        },
            ...
            function ensureOdp() {
                if (_app.odp.enabled()) {
                    ...
                    if (odp && odp.invoke({ method: methodName, dataView: dataView, params: params, deferred: deferred }))
                        runOnServer = false;
                    if (!runOnServer) {
                        var paramArgs = params.data.args;
                        if (paramArgs && paramArgs.CommandName === 'AutoFill')
                            runOnServer = true;
                    }
                }
            }

            params.processData = false; // jQuery 3.5.1 and 3.6.0 are replacing %20 with +, which is problematic for values that look like this: %js%20
        ...
        commit: function (options) {
            ...
            function uploadBlobsToServer() {
                ...
                if (fileUploads.length) {
                    _touch.notify({ text: String.format(resourcesODP.UploadingFiles, fileUploads.length) });
    ...
    _app.OfflineDataProcessor.prototype = {
        ...
        invoke: function (data) {
            ...
            function executeOnClient() {
                if (method.match(/^(GetPage|Execute|GetListOfValues)$/) && that.tracking(dataView)) {
                    controller = data.params.data.controller;
                    $.when(_odp.getControllers(controller)).done(function () {
                        ...
                        if (commandName !== 'Update' || rootChanged) {
                            ...
                            data.deferred.done(function (r) {
                                ...
                                if (result.Errors.length) {
                                    that.snapshot('restore');
                                    if (!dataView.get_parentDataView())
                                        dataView.odp = null;
        ...
        select: function (options) {
            ...
            if (options.yield)
                ...
                var filterEvent = $.Event('viewfilter.odp.app', { odp: that, controller: that._dataView._controller, view: that._dataView._viewId });
                $(document).trigger(filterEvent);
                    ...
                    pageFilter.forEach(function (filterExpression) {
                        ...
                        if (filterMatch) {
                            ...
                            if (valueMatch) {
                                ...
                                var field = _odp.controllers[page.Controller]._map.fields[alias];// page._fieldMap[alias];
                                if ((field != null && field.allowQBE !== false || operation === "~") && (page.DistinctValueFieldName !== field.name || matchListCount > 0 || operation === "~" || page.AllowDistinctFieldInFilter /*|| page.CustomFilteredBy(field.Name)*/) || deepSearching) {
                                    ...
                                    if (deepSearching) {
                                        ...
                                        var textDateNumber = '\\p{L}\\d' + RegExp.escape(dateTimeFormat.DateSeparator + dateTimeFormat.TimeSeparator + numberFormat.NumberDecimalSeparator);
                                        // (Token((Quote)(Value))|((Quote)(Value)|(())|(Value))
                                        //    1       3     4         6       7          10
                                        // Token (1), Quote (3, 6), Value (4, 7, 10)
                                        var quickFindRegex = new RegExp("\\s*(((\")(.+?)\")|((\\u{27})(.+?)\\u{27})|(,|;|(^|\\s+)-)|([" + textDateNumber + "]+))", 'giu');
    ...
    function removeExpiredBlobChangeLogTimestamps() {
        if (!removeExpiredBlobChangeLogTimestamps.done) {
            removeExpiredBlobChangeLogTimestamps.done = true;
            var timestampUseLimit = _odp.offline() ? 24 : 1;
            setTimeout(function () {
                var changed = false;
                for (var handlerAndKey in blobChangeLog)
                    if ((new Date().getTime() - blobChangeLog[handlerAndKey]) / 1000 > 60 * 60 * timestampUseLimit) {
                        // delete the timestamp from the log if it is older than the specified number of hours (24 - offline / 1 - online)
                        delete blobChangeLog[handlerAndKey];
                        changed = true;
                    }
                if (changed)
                    _app.userVar('blobChangeLog', blobChangeLog);
