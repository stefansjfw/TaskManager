...
(function () {
    ...
    function notifyDataFilterChanged(dataView) {
        ...
        _touch.notify({ dataView: dataView, text: text || resourcesMobile.FilterCleared });
    }
    ...
    $document.on('searchperform.dataview.app', function (e) {
        ...
        if (trigger) {
            // clear the  session-level lists in the lookups whenever any value or operation has changed
            var keysToRemove = []
            var triggerPrefix = dataView._viewId + '_' + trigger + '_';
            for (var key in dataView._pageSession)
                if (key.match(/(_listCache|_listOfValues_)/) && !key.startsWith(triggerPrefix))
            ...
            if (trigger.match(/_op$/)) {
                ...
                if (field) {
                    ...
                    if (targetControllerChanged) {
                        ...
                        else if (fieldValue != null && field.ItemsStyle) {
                            field.Items = [[fieldValue, field.format(fieldValue)]];
    ...
    _app._search = function (method, options) {
        ...
        function parse() {
            ...
            if (options.showErrors && result.errors.length)
                _app.input.focus({ fieldName: result.errors[0].field, message: result.errors[0].error });
            return result;
        }
        ...
        function enumerateFieldsForTopValues() {
            ...
            if (!list.length)
                allFields.forEach(function (f) {
                    ...
                    if (f.ItemsDataController && list.length < 3 && aliasField.Type == 'String' && !f.ItemsTargetController && !(f.AllowAutoComplete == false || f.AllowMultipleValues == false))
        ...
        function changeMatch() {
            ...
            match.fields.forEach(function (cf) {
                ...
                if (f.Extended && f.Extended.context) {
                    ...
                    f._dataView.sessionRemove(_app.lovKey(f)/*f.Name + '_listOfValues_'*/);
                }
            });
                ...
                match.fields.forEach(function (mf, index) {
                    ...
                    if (dataType === 'text') {
                        var allowAutoComplete = originalField.AllowAutoComplete !== false,
                            ...
                            isBasket = f.ItemsTargetController;
                        q.items = {
                            controller: isBasket ? f.ItemsDataController : dataView._controller,
                            view: isBasket ? null : dataView._viewId,
                            dataValueField: isBasket ? f.ItemsDataTextField : fieldName,
                            dataTextField: isBasket ? f.ItemsDataTextField : fieldName,
                        ...
                        q.autoCompletePrefixLength = originalField.AutoCompletePrefixLength;
                        ...
                        q.options = {
                            lookup: {
                                distinct: isBasket ?
                                    false :
                                    (
                                        (dataView._fields[0] != f && !f.is('lookup-distinct-none') ||
                                            f.is('lookup-distinct')) ||
                                        f.is('search-sample-distinct') || // TODO - Consider remove the "search-sample-distinct" dependency. It is enough to tag the field as "lookup-distinct".
                                        !!allFields[f.OriginalIndex].ItemsStyle
                                    ) && !f.is('search-sample-all'),      // TODO - Consider remvoving the 'search-sample-all'' dependency. Use "lookup-distinct-none" instead.
                                acceptAnyValue: true,
        ...
        function quickFindAutoComplete() {
            ...
            if (list.length < maxItemCount && qryRegex && !dataView.tagged('search-suggestions-none') && keyboard) {
                ...
                if (!dataInput.data('scannedDataItems')) {
                    dataItems = $('#' + dataView._id).find('.dv-item .ui-btn');
                    if (!dataItems.length)
                        dataItems = $('[data-for="' + dataView._id + '"]').find('.dv-item .ui-btn');
        ...
        if (mode === 'advanced' || options.field /*|| searchOnStart && mode == 'advanced' && method != 'toggle'*/) {
            ...
            survey.tryLoad = false;// !surveyContext.field;
            _app.survey(survey);
        }
        else {
