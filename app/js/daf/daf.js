...
(function () {
    ...
    String.htmlEncode = function (s) {
        return typeof s == 'string' && s.match(_app._htmlTest) ? _app.htmlEncode(s) : s;
    };
    ...
    function raiseError(message) {
        if (_touch)
            _touch.notify({ text: message, duration: 60000 });
        throw Error(message);
    }
    ...
    _app.htmlToText = function (html) {
        ...
        return converter.html(_app.safeHtml(html)).text().replace(/></g, '> <');
    };
    ...
    _app.cookie = function (name, value, days, hours, minutes) {
        var expires = '',
            path = __baseUrl;
        if (days || hours || minutes) {
            var date = new Date();
            date.setTime(date.getTime() + (days * 24 * 60 * 60 * 1000) + ((hours || 0) * 60 * 60 * 1000) + ((minutes || 0) * 60 * 1000));
            expires = "; expires=" + date.toGMTString();
        }
        if (path.match(/.\/$/))
            path = path.substring(0, path.length - 1);
        document.cookie = name + "=" + value + "; SameSite=Strict" + expires + "; path=" + path;
    };
    ...
    _app.userVar = function (name, value) {
        var originalName;
        if (name.match(/\?$/))
            originalName = name = name.substring(0, name.length - 1);
        if (!userScope)
        ...
        if (arguments.length === 1) {
            try {
                ...
                if (typeof value == 'undefined' && originalName)
                    value = _app.storage.get(__settings.appName.replace(/\W/g, '_') + '___' + originalName);
                if (value != null)
    ...
    _app.urlArgs = function urlArgs(url) {
        if (!url)
            url = _app._commandLine || location;
        if (url.href)
            url = url.href;
        var args = {};
        var iterator = /(\w+)=(.*?)(&|#|$)/g;
    ...
    // unresolved implementation of $app.restful will load the API and execute the first call
    _app.restful = function (options) {
        options = options || {};
        return new Promise(function (resolve, reject) {
            _app.getScript('~/js/sys/restful', {
                then: function () {
                    _app.restful(options)
    ...
    _app.getScript = function (url, options) {
        ...
        var callback,
            ...
            hash = '',
            manifest = options.manifest || _app.manifest,
            physicalUrl;

        function resolveSelf() {
            countDown--;
            if (countDown <= 0) {
                script.forEach(function (scriptInfo) {
                    if (scriptInfo.callback)
                        scriptInfo.callback();
                    scriptInfo.resolve();
                });
                _scripts[url] = { ok: true };
            }
        }
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (!manifest) {
                ...
                if (manifest.length) {
                    var manifestCallbacks = _scripts['manifest.json'];
                    if (!manifestCallbacks) {
                        manifestCallbacks = _scripts['manifest.json'] = [];
                        fetch(manifest.attr('href')).then(function (response) {
                            return response.text();
                        }).then(function (json) {
                            _app.manifest = JSON.parse(json);
                            var pendingCount = manifestCallbacks.length;
                            manifestCallbacks.forEach(function (pendingCallback) {
                                _app.getScript(pendingCallback.url, pendingCallback.options)
                                    .then(function () {
                                        pendingCount--;
                                        if (!pendingCount)
                                            resolve();
                    ...
                    manifestCallbacks.push({ url: url, options: options })
                    return false;
                }
                else
                    manifest = null;
            ...
            if (manifest) {
                var testUrl = url.match(/^~\//) ? url.substring(2) : url;
                manifest.offline_resources.every(function (r) {
                    var found = r.src === testUrl;
                    if (found)
                        hash = '?h=' + r.hash;
                    return !found;
                });
            ...
            var scriptInfo = { resolve: resolve, reject: reject, callback: callback };

            if (script == null && $('script[src="' + url + hash + '"]').length)
                script = _scripts[url] = { ok: true };

            if (script && 'ok' in script) {
                if (script.ok) {
                    if (callback)
                        callback();
                    resolve();
                ...
                script.push(scriptInfo);
            else {
                script = _scripts[url] = [scriptInfo];
                ...
                physicalUrl = url + hash;
                head = document.head;
                if (head.append/* && !_app.host*/) {
                    if (isJavaScript) {
                        resourceElem = document.createElement('script');
                        resourceElem.src = physicalUrl;
                    }
                    else {
                        resourceElem = document.createElement('link');
                        resourceElem.href = physicalUrl;
                        resourceElem.rel = 'stylesheet';
                    }
                    resourceElem.onload = resolveSelf;
                    resourceElem.onerror = rejectSelf;
                    ...
                    if (isJavaScript)
                        ...
                        $htmlTag('link', '', 'href="' + physicalUrl + '" rel="stylesheet" ').appendTo('head');
            }
        });
    };

    _app.getScript.controllers = {};
    ...
    _app.prototype = {
            ...
            if (method === 'row') {
                ...
                allFields.forEach(function (f) {
                    var index = f.Index,
                        name = f.Name,
                        value;
                    if (!(name === 'sys_pk_' || name === 'Status' && f._system && index === allFields.length - 1)) {
        ...
        get_selectedValue: function () {
            ...
            var sv = _touch ? null : this._get_selectedValueElement();
            return sv ? sv.value : '';
        },
        ...
        get_lookupInfo: function () {
            return this._lookupInfo;
        },
        set_lookupInfo: function (value) {
            this._lookupInfo = value;
        },
            ...
            if (!isNullOrEmpty(field.ContextFields)) {
                ...
                while (m) {
                    ...
                    if (m2) {
                        ...
                        if (m2)
                            contextFilter.push({ Name: m[1], Value: m2[1], Literal: true });
                    }
                    else {
                        ...
                        if (f) {
                            ...
                            for (i = 0; i < contextValues.length; i++) {
                                if (contextValues[i].Name == f.Name) {
                                    ...
                                    contextFilter.push({ Name: m[1], Value: fieldValue });
                                    found = true;
                                    break;
                                }
                            }
                            if (!found)
                                for (i = 0; i < contextValues.length; i++) {
                                    if (contextValues[i].Name == f.Name + '_op') {
                                        contextFilter.push({ Name: m[1] });
                                        break;
                                    }
                                }
                        }
            ...
            if (!expression._variables) {
                ...
                while (m) {
                    ...
                    if (!found) {
                        var f = this.findField(fieldName);
                        if (f)
                            Array.add(vars, { 'name': fieldName, 'regex': new RegExp('(\\[' + fieldName + '\\])|((\\$row|this)\\.' + fieldName + ')', 'g'), 'replace': String.format('this._javaScriptRowValue({0})', f.Index) });
                    }
                    m = re.exec(expression.Test);
                }
            ...
            try {
                if (typeof whenClientScript == 'string') {
                    ...
                    if (whenClientScript) {
                        ...
                        if (!whenClientScript)
                            whenClientScript = _app.functions[clientScript] = action.WhenClientScript = eval('(function(){return ' + that._prepareJavaScriptExpressionEx(clientScript) + '})');
                    }
                }
                var result =
                    (!action.WhenViewRegex || action.WhenViewRegex.exec(that.get_viewId()) != null === action.WhenViewRegexResult) &&
                    (!action.WhenTagRegex || action.WhenTagRegex.exec(that.get_tag()) != null === action.WhenTagRegexResult) &&
                    (!action.WhenHRefRegex || action.WhenHRefRegex.exec(location.pathname) != null === action.WhenHRefRegexResult) &&
                    (ignoreScript || !whenClientScript || !!whenClientScript.call(that, that));

                return result;
            }
            catch (ex) {
                raiseError(ex.message + (clientScript ? '\n' + clientScript : ''));
            }
        },
        ...
        _renderFilterDetails: function (sb, currentFilter, includeBanner) {
            ...
            for (i = 0; i < currentFilter.length; i++) {
                ...
                while (m && (m[1].startsWith('~') || !(field.Index === field.AliasIndex && field.IsPrimaryKey && field.Hidden))) {
                    ...
                    if (m[1].startsWith('~')) {
                        sb.appendFormat(String.format('{0} <b class="String">{1}</b>', resources.InfoBar.QuickFind, _app.htmlEncode(that.convertStringToFieldValue(field, m[3]))));
                        ...
                        if (!fd) {
                            ...
                            if (!String.isJavaScriptNull(m[2])) {
                                ...
                                item = field.ItemsTargetController ? null : that._findItemByValue(field, v);
        ...
        refreshData: function () {
            var that = this,
                pageIndex = -2;
            if (!that._busy()) {
                that._fields.forEach(function (f) {
                    if (f.ItemsTargetController)
                        pageIndex = -1;
                });
                that._clearCache();
                that.set_pageIndex(pageIndex);
                that._loadPage();
            }
        },
        ...
        canSync: function () {
            var that = this,
                master = that.get_master(),
                selectedKey = master ? master.get_selectedKey() : null,
                externalFilter = that._externalFilter,
                externalFilterFirstValue = master && externalFilter && externalFilter.length ? externalFilter[0].Value : null;
            return !that._busy() && (!master || !externalFilter.length || selectedKey.length && selectedKey[0] != null && externalFilterFirstValue && externalFilterFirstValue != '%js%');
        },
            ...
            if (that.get_mode() != Web.DataViewMode.View) {
                ...
                if (skipInvoke)
                    ...
                    if (survey && survey.result) {
                        if (survey.sharedInstance) {
                            var backup = that._backup = {};
                            // backup the dataview properties
                            for (var key in that)
                                backup[key] = that[key];
                        }
        ...
        _invoke: function (methodName, params, onSuccess, userContext, onFailure) {
            var that = this,
                ...
                user = _app.AccountManager.current(),
                headers = {},
                alternativeInvokeEvent = $.Event('invoke' + methodName.toLowerCase() + '.app', { invoke: { params, success: onSuccess, failure: onFailure, headers } });

            if (user && user.access_token && !methodName.match(/^Login|Logout$/))
            ...
            $document.trigger(alternativeInvokeEvent);
            if (alternativeInvokeEvent.isDefaultPrevented())
                return;

            if (servicePath.match(/\.asmx/))
                that._wsRequest = _Sys.Net.WebServiceProxy.invoke(servicePath, 'Invoke', false, { method: methodName, args: params },
                    function (result, context, method) {
                        if (result && result.ExceptionType)
                            that._onMethodFailed(createInvokeError(methodName, result), null, userContext);
                        else
        ...
        _onGetPageComplete: function (result) {
            var that = this,
                ...
                i, expressions,
                resultEvent = $.Event('getpagecomplete.dataview.app', { dataView: that, response: result }),
                controllerScripts = _app.getScript.controllers;
            requiredScript = controllerScripts[that._controller];
            if (requiredScript) {
                _app.getScript(requiredScript).then(function () {
                    delete controllerScripts[that._controller];
                    that._onGetPageComplete(result);
                });
                return;
            }
            $document.trigger(resultEvent);
            if (resultEvent.promise) {
                resultEvent.promise.then(function () {
                    that._onGetPageComplete(result);
                });
                return;
            }
                    ...
                    for (i = 0; i < resultFields.length; i++) {
                        ...
                        if (field.Type === 'Boolean') {
                            if (!fieldItems.length) {
                                fieldItems = field.Items = Array.clone(field.AllowNulls && !(_touch && itemsStyle === 'CheckBox') ? resourcesData.BooleanOptionalDefaultItems : resourcesData.BooleanDefaultItems);
                                if (!itemsStyle)
                            ...
                            if (_touch) {
                                fieldItems.splice(fieldItems.length - 2, 0, fieldItems[fieldItems.length - 1]);
                                fieldItems.splice(fieldItems.length - 1, 1);
                                if (!field.Columns && itemsStyle == 'RadioButtonList')
                                    field.Columns = fieldItems.length;
                            }
                        }
                ...
                for (i = 0; i < groups.length; i++) {
                    ...
                    if (g.Id == groupId) {
                        ...
                        for (j = 0; j < actions.length; j++) {
                            a = actions[j];
                            if (a.Id == actionId)
                                return { action: a, actionIndex: j, group: g, groupIndex: i, dataViewId: that._id };
                        }
                    }
                }
            }
        ...
        findAction: function (path) {
            var action = this.actionInfo(path);
        ...
        _onMethodFailed: function (err, response, context) {
            ...
            _touch.notify({
                text:
                    navigator.onLine ? String.format('{0}\n\n{1}/{2} error {3}:\n\n{4}',
                        err.get_message(),
                        that._controller,
                        that._viewId,
        ...
        _createArgsForListOfValues: function (distinctFieldName) {
            var that = this,
                f = that.findField(distinctFieldName),
                lc = that.get_lookupContext(),
                filter = that._searchBarVisibleIndex == null ? that.get_filter() : that._createSearchBarFilter(true),
                request = {
                    FieldName: distinctFieldName, Controller: that.get_controller(), View: that.get_viewId(),
                    Filter: that._combinedFilter(filter.length === 1 && filter[0].match(/(\w+):/)[1] === distinctFieldName ? null : filter),
                    ExternalFilter: that.get_externalFilter(),
                    LookupContextFieldName: lc ? lc.FieldName : null, LookupContextController: lc ? lc.Controller : null,
                    LookupContextView: lc ? lc.View : null, Tag: that.get_tag(),
                    QuickFindHint: _touch ? that.viewProp('quickFindHint') : null
                };
            if (f.ItemsTargetController) {
                return {
                    controller: f.ItemsDataController, request: { FieldName: f.ItemsDataTextField, Controller: f.ItemsDataController }
                }
            }
            else {
                that._useSearchParams(request);
                return { controller: that.get_controller(), view: that.get_viewId(), request: request };
            }
        },
            ...
            if (pendingUploads && pendingUploads.length) {
                ...
                if (upload.files) {
                    ...
                    result = _app.upload('execute', {
                        ...
                        stopWaitng();
                        pendingUploads.splice(0, 0, upload); // return the blob in the queue
                        var values = [{ name: keyFieldName, oldValue: key }];
                        if (that._lastCommandName === 'New')
                            // delete the previously created record
                            _app.execute({ controller: that._controller, view: that._viewId, command: 'Delete', values: values });
                        else {
                            // restore the field values of the udpated record
                            var data = that.data('modified');
                            values[0].oldValue = data[keyFieldName];
                            for (var modifiedFieldName in data._modified)
                                if (modifiedFieldName !== blobFieldName)
                                    values.push({ name: modifiedFieldName, newValue: data._modified[modifiedFieldName], oldValue: data[modifiedFieldName], modified: true });
                            _app.execute({ controller: that._controller, view: that._viewId, command: 'Update', values: values });
                        }
                        _app.alert(response.responseText || resourcesMobile.Error);
                    });
                }
                //else
        ...
        _onExecuteComplete: function (result, context) {
                    ...
                    if (existingRow) {
                        ...
                        if (_touch) {
                            ...
                            if (isForm) {
                                ...
                                originalValues = that._collectFieldValues();
                                parentDataView = that.get_parentDataView(null);
                                var parentCommandRow, editRow,
                                    parentExtension;
                                if (parentDataView && parentDataView !== that && parentDataView._controller === that._controller) {
                                    parentCommandRow = [];
                        ...
                        if (that._closeInstantDetails()) {
                            ...
                            for (i = 0; i < actions.length; i++) {
                                ...
                                if (a.WhenLastCommandName === lastCommandName && (!a.WhenLastCommandArgument || a.WhenLastCommandArgument === lastCommandArgument) && that._isActionMatched(a)) {
                                    commandName = a.CommandName;
                                    commandArgument = a.CommandArgument;
                                    commandIsSelectOrEdit = !!commandName.match(/^(Edit|Select)/);
                                    if (_touch)
                                        dataView = that.delegateCommand(commandName, commandArgument);
                                    if (!dataView) {
                                        if (_touch && commandIsSelectOrEdit) {
                                            that.discard();
                                            var deferred = that._deferred;
                                            that._deferred = null;
                                            _touch.goBack(function () {
                                                ...
                                                var showArgs = {
                                                    ...
                                                    startArgument: commandArgument,
                                                    defaultValues: that._defaultValues
                                                };
                                                showArgs.filter = {};
                                                var data = that.data();
                                                that._keyFields.forEach(function (f) {
                                                    showArgs.filter[f.Name] = data[f.Name];
                                                });
                                                if (deferred)
                                                    setTimeout(_touch.show, 0, showArgs);
                                            });
                                            return;
                                        }
                                        dataView = that;
                                    ...
                                    if (!a.WhenKeySelected || (dataView.get_selectedKey() || []).length) {
                                        ...
                                        dataView._pendingSelectedEvent = commandIsSelectOrEdit;
            ...
            if (noErrors) {
                ...
                if (parentDataView && parentDataView._controller === that._controller)
        ...
        _prepareJavaScriptExpressionEx: function (script) {
            ...
            var newScript = [];
            var iterator = /(\$row|\$master|this)\.(\w+)/gm;
            var m = iterator.exec(script);
            var lastIndex = 0;
            while (m) {
                newScript.push(script.substring(lastIndex, m.index));
                var usingThis = m[1] === 'this';
                var doReplace = !usingThis || this.findField(m[2]);
                if (doReplace) {
                    newScript.push('this.fieldValue("' + m[2] + '"');
                    if (m[1] === '$master')
                        newScript.push(',"master"');
                    newScript.push(')');
                }
                else if (usingThis && this[m[2]] == null)
                    raiseError('"' + m[2] + '" is not a valid field or member of the DataView instance: ' + script);
                else
                    newScript.push(m[0]);
            ...
            if (lastIndex) {
                newScript.push(script.substring(lastIndex));
                script = newScript.join('');
            }
            if (script.length === originalLength)
                script = script.replace(/\[((\w+)\.)?(\w+)\]/g, 'this.fieldValue("$3","$2")');
            return script;
            //    return script
            //        .replace(/\[((\w+)\.)?(\w+)\]/g, 'this.fieldValue("$3","$2")')
            //        .replace(/(\$row|this2)\.(\w+)/gm, 'this.fieldValue("$2")')
    ...
    _app.showMessage = function (message) {
        _touch.notify({ text: message, duration: 'medium' });
    };
    ...
    _app.resolveClientUrl = function (url, baseUrl) {
        if (!baseUrl)
            baseUrl = _window.__baseUrl || (typeof appBaseUrl == 'string' ? appBaseUrl : '');
    ...
    _app.jsExt = function () {
        ...
        if (!manifest)
            extension += '?_v=' + __settings.version;
        return extension;
    };
    ...
    _app.execute = function (args) {
        ...
        dataView._onGetPageComplete = function (result) {
            args.controller = result.Controller;
            if (!batchMode)
                done();
                ...
                $.ajax({
                    ...
                    if (!handledByRule)
                        if (result.Success) {
                            ...
                            _touch.notify(result.Errors.join('\n'));
                    if (callback)
                        callback();
                });
            }
    ...
    _app.eval = function (text, data, propSetValue) {
        if (typeof text == 'string') {
            if (arguments.length >= 2) {
                if (data != null && text.match(/\$\{|this(\??\.|\[)|\n/)) {
                    var func = _functions[text],
                        isInline;
                    if (func == null) {
                        isInline = !text.match(/\n|\s*=\s*value(\s|;|$)/);
                        func = '(function(value){';
                        func += isInline && text.match(/\$\{/) ?
                            'var r=\`' + text + '\`;return r==\'undefined\'?null:r;' :
                            isInline && text.match(/this\??\./) ?
                                'return ' + text :
                                text;
                        try {
                            func = _functions[text] = eval(func + '})');
                        }
                        catch (ex) {
                            if (_touch)
                    ...
                    return func.call(data, propSetValue);
                }
            }
            else {
    ...
    _app.showModal = function (parent/*anchor*/, controller, view, startCommandName, startCommandArgument, baseUrl, servicePath, filter, properties) {
        ...
        if (survey) {
            ...
            if (Array.isArray(surveyText))
                surveyText = survey.text = surveyText[0];
            surveyText2 = survey.text2;
            if (surveyText2 && typeof surveyText2 == 'function')
                surveyText2 = survey.text2 = surveyText2.call(dataView);
            if (surveyText && typeof surveyText == 'function')
                surveyText = survey.text = surveyText.call(dataView);
            if (!surveyText && surveyText2) {
                surveyText = survey.text = surveyText2;
                surveyText2 = survey.text2 = null;
            }
            if (surveyText2)
                if (_touch)
                    survey.text = [surveyText, surveyText2];
                else
            ...
            if (result && result != 'false') {
                if (result != "true") {
                    if (password && !password.match(/(.+?:.+?;){2,}/)) {
                        ...
                        if ((result.claims || {})['urn:app:remember-me'])
    ...
    function executeExternalJavaScriptBusinessRule(dataView, args, beforePhase) {
        ...
        if (rules) {
            if (!beforePhase)
                rules = rules.after;
            args.phase = beforePhase ? 'before' : 'after';
            if (rules) {
                if (commandName in rules) {
                    rules = rules[commandName];
                    if (commandArgument && commandArgument in rules)
    ...
    _businessRules.prototype = {
        ...
        process: function (phase) {
            ...
            try {
                    ...
                    for (i = 0; i < expressions.length; i++) {
                        ...
                        if (exp.Type === Web.DynamicExpressionType.ClientScript && exp.Scope === Web.DynamicExpressionScope.Rule) {
                            ...
                            if (m) {
                                ...
                                if (!ruleFunc)
                                    try {
                                        var ruleText = that._parseRule(ruleName, m[6]);
                                        ruleFunc = _businessRules._compiled[ruleText];
                                        if (!ruleFunc)
                                            ruleFunc = _businessRules._compiled[ruleText] = eval(ruleText);
                                ...
                                Array.add(that._rules, { 'commandName': m[2], 'commandArgument': m[3], 'view': m[4], 'phase': m[5], 'script': ruleFunc, id: m[1] });
                            }
                        }
                    }
                }
                for (i = 0; i < that._rules.length; i++) {
                    var r = currentRule = that._rules[i];
                    if (r.phase !== phase)
                        continue;
                    var skip = false;
                ...
                if (currentRule)
                    ex.message = 'Rule "' + currentRule.id + '". ' + ex.message;
                if (_touch) {
                    _touch.notify(ex);
                }
                throw ex;
    ...
    _app.dataUrlToBlob = function (url) {
        var byteString,
            ...
            byteArray, i,
            urlInfo = url.split(',');
        if (urlInfo[0].indexOf('base64') >= 0)
            byteString = atob(urlInfo[1]);
        else
            byteString = decodeURI(urlInfo[1]);
        mimeString = urlInfo[0].split(':')[1].split(';')[0];
        ...
        function initialize() {
            if (!container.is('.app-drop-box')) {
                ...
                var dataView = findDataView(dataViewId),
                    ...
                    hadValue = value && !value.toString().match(/null\|/);
                    ...
                    $(_touch ? $span('material-icon app-icon app-clear') : $a('app-clear ui-btn ui-btn-inline ui-corner-all')).insertAfter(container).text(_touch ? 'delete' : resourcesFiles.Clear).attr('title', resourcesFiles.Clear)
                        .on(clickEvent, function (e) {
                            ...
                            _app.confirm(resourcesFiles.ClearConfirm).then(function () {
                                container.removeClass('app-dragging').addClass('app-empty').trigger('focus');
                                resizeSignature();
                                if (dataViewId) {
                                    var dataView = findDataView(dataViewId);
                                ...
                                container.removeClass('app-dragging').trigger('focus');
                            });
                        });
                    if (!_touch)
                        resizeSignature();
    ...
    _app.upload.multi = {
        ...
        send: function (options) {
            ...
            function finished(message) {
                _touch.busy(false);
                _touch.notify(message ? { text: message, duration: 'long' } : false);
            ...
            if (index < files.length) {
                _touch.busy(true);
                f = files[index];
                _touch.notify({ text: String.format(resourcesMobile.Sync.Uploading, f.name), duration: 60000 });
                if (options.name) {
                    nameVal = f.name;
    ...
    _app.uploadFileAjax = function (options) {
        ...
        // The 'file' value is either an object, an array, or an instance of FileList
        if (!('length' in files))
            files = [files];
        ...
        _app.restful({ url: options.url, method: 'POST', body: formData, dataType: 'response' })
            .then(function (result) {
    ...
    // **************************************
    // TODO: native unicode implementation
    // **************************************

    //function prettyText(s, capitalize) {
    //    if (s.match(/\p{Ll}/u) && s.match(/\p{Lu}/u)) {
    //        s = s.split(/(?:(\d+))/).join(' ');
    //        var iterator = /\p{Lu}+/ug,
    //            result = [], i = 0,
    //            m = iterator.exec(s);
    //        while (m) {
    //            result.push(s.substr(i, m.index - i));
    ...
    _app.safeHtml = function (html) {
        if (html != null)
            html = html.replace(/(<)(\/)?(script|iframe|link)(.*?)(>)/ig, '&lt;$2$3$4&gt;');
        return html;
    };
    ...
    */
    ...
    // unresolved implementation of $app.survey will load the API and execute the first call

    _app.survey = function (method, options) {
        options = options || {};
        //return new Promise(function (resolve, reject) {
        _app.getScript('~/js/daf/daf-survey', {
            then: function () {
    ...
    */
    ...
    _app.AccountManager = {
        ...
        set: function (user) {
            if (user.UserName) // ensure compatibility with the serialization of UserTicket in the *.aspx projects 
            ...
            var that = this,
                ...
                avatars,
                userData = { name: user.name, email: user.email, access_token: user.access_token, refresh_token: user.refresh_token, claims: user.claims, session: !!user.session, Handler: user.Handler };
